...
for i in range(5):
    ...
    print("---")
# =========================
#  ĐÁNH GIÁ CHỈ SỐ METRICS
# =========================
from sklearn.metrics import mean_absolute_error, mean_squared_error, r2_score
from prettytable import PrettyTable
import numpy as np

# 1. Tách riêng Temp và Humidity để đánh giá (vì đơn vị khác nhau)
# Index 0: Temperature, Index 1: Humidity
real_temp = y_test_unscaled[:, 0]
pred_temp = y_pred_unscaled[:, 0]

real_hum = y_test_unscaled[:, 1]
pred_hum = y_pred_unscaled[:, 1]
...
# Tính cho Temperature
mae_t, mse_t, rmse_t, r2_t = calculate_metrics(real_temp, pred_temp)

# Tính cho Humidity
mae_h, mse_h, rmse_h, r2_h = calculate_metrics(real_hum, pred_hum)

# 3. Hiển thị bảng PrettyTable
table = PrettyTable()
table.field_names = ["Metric", "Temperature (°C)", "Humidity (%)"]

table.add_row(["MAE",  f"{mae_t:.4f}", f"{mae_h:.4f}"])
table.add_row(["MSE",  f"{mse_t:.4f}", f"{mse_h:.4f}"])
table.add_row(["RMSE", f"{rmse_t:.4f}", f"{rmse_h:.4f}"])
table.add_row(["R2 Score", f"{r2_t:.4f}", f"{r2_h:.4f}"])
...
# =========================
# 14. LƯU KẾT QUẢ VÀO FILE LOG & GIỮ MÀN HÌNH
# =========================

# Lưu bảng kết quả vào file text
result_path = "lstm_training_results.txt"
with open(result_path, "w", encoding="utf-8") as f:
    f.write("=== KẾT QUẢ ĐÁNH GIÁ MÔ HÌNH ===\n")
    f.write(table.get_string()) # Lấy nội dung bảng thành chuỗi
    f.write("\n\n")
    f.write("=== VÍ DỤ DỰ BÁO ===\n")
    f.write(f"Dự báo 15 phút tới: {y_future}\n")
    f.write(f"Trạng thái: {c_text}\n")
